  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        setError('Ensure backend is running on port 5000.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <section id="projects" className="py-24 bg-white dark:bg-gray-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div 
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-base text-primary font-bold tracking-wide uppercase">Portfolio</h2>
            ...
            {projects.map((project) => (
              ...
              >
                ...
                <div className="relative h-48 w-full overflow-hidden bg-gray-200 dark:bg-gray-700">
                  ...
                  <img 
                    src={project.imageUrl || `https://placehold.co/600x400/2563eb/ffffff?text=${encodeURIComponent(project.title)}`} 
                    alt={project.title}
                  ...
                  <div className="absolute bottom-4 right-4 flex space-x-3 z-20 translate-y-10 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <a href={project.link} target="_blank" className="p-2 bg-white text-gray-900 rounded-full hover:bg-primary hover:text-white transition-colors shadow-lg">
                ...
                <div className="flex-1 p-6 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {project.tags.map((tag) => (
                      <span key={tag} className="px-2.5 py-1 text-xs font-medium rounded-md bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 border border-blue-100 dark:border-blue-800">
                        {tag}
                      </span>
                    ))}
                  </div>
