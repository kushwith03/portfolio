    ...
    <section className="fixed inset-0 pointer-events-none z-10 flex items-center justify-center md:pl-56 px-6 md:px-24">
      ...
      >
        <div className="flex flex-col gap-4 items-center text-center">
          ...
          <p className="text-white/30 text-[10px] uppercase tracking-[0.4em] font-bold mt-2">
            // A production-ready stack optimized for scalability and intelligence.
          </p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-white/5 border border-white/5">
          {DOMAINS.map((domain, i) => (
            <div key={i} className="bg-black/40 p-10 flex flex-col gap-8 group pointer-events-auto cursor-default hover:bg-white/[0.02] transition-colors duration-700">
